...
from script import get_places,unpack_places,Place,centroid,haversine_distance,distance_based_decision
...
def get_location(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            user_places=[Place(unpack_places(get_places(ad))) for ad in user_addresses]
            target_places=Place(unpack_places(get_places(address_4)))
            result=distance_based_decision(1,[target_places],user_places)
            ready=True
            print(result)
